...
<template>
  <validation-provider v-slot="{ errors, required }" v-bind="veeProviderProps">
    <v-checkbox v-bind="$attrs" :class="{ required, 'required-marker': required && !disabledRequiredMarker }" :disabled="disabled" :error-messages="errors" :label="label" :readonly="readonly" v-on="$listeners">
