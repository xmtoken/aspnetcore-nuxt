...
<template>
  <validation-provider v-slot="{ errors, required }" v-bind="veeProviderProps">
    <v-select v-bind="$attrs" :class="{ required, 'required-marker': required && !disabledRequiredMarker }" :disabled="disabled" :error-messages="errors" :label="label" :menu-props="menuProps" :readonly="readonly" v-on="$listeners">
