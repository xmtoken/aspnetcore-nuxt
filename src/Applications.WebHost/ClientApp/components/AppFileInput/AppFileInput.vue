...
<template>
  <validation-provider v-slot="{ errors, failed, required }" v-bind="veeProviderProps">
    <v-file-input ref="field" v-bind="$attrs" :class="{ ...classes, required, 'required-marker': required && !disabledRequiredMarker }" :clearable="internalClearable" :disabled="disabled" :error-messages="errors" :hide-details="internalHideDetails" :label="label" :readonly="readonly" v-on="$listeners" @blur="onBlur" @focus="onFocus">
