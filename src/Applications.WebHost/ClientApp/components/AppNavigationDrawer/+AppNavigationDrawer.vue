          ...
          routes: [
            {
              ...
              routes: [
                {
                  text: 'AppTextField',
                  path: '/samples/components/app-text-field',
                },
              ],
            },
          ],
    ...
    for (const lv1route of this.routes) {
      if (lv1route.routes) {
        for (const lv2route of lv1route.routes) {
          if (lv2route.path) {
            lv1route.expands = lv1route.expands || isRouteExpands(lv2route.path);
          }
          if (lv2route.routes) {
            for (const lv3route of lv2route.routes) {
              if (lv3route.path) {
                lv1route.expands = lv1route.expands || isRouteExpands(lv3route.path);
                lv2route.expands = lv2route.expands || isRouteExpands(lv3route.path);
              }
            }
          }
        }
...
<template>
  <v-navigation-drawer app dark fixed :mini-variant="isNavigationDrawerMiniVariant">
