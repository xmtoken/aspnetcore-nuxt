...
<template>
  <validation-provider v-slot="{ errors, required }" v-bind="veeProviderProps">
    <v-textarea v-bind="$attrs" :class="{ required, 'required-marker': required && !disabledRequiredMarker }" :error-messages="errors" :label="label" :value="internalValue" v-on="listeners" @change="onChange" @input="onInput">
