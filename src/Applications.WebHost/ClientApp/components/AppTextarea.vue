...
<template>
  <validation-provider v-slot="{ errors, required }" v-bind="veeProviderProps">
    <v-textarea v-bind="$attrs" :class="{ required, 'required-marker': required && !disabledRequiredMarker }" :disabled="disabled" :error-messages="errors" :label="label" :readonly="readonly":value="internalValue" v-on="listeners" @change="onChange" @input="onInput">
